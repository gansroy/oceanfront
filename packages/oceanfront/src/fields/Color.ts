...
export const ColorField = defineFieldType({
  ...
  setup(props: FieldProps, ctx: FieldContext) {
    ...
    watch(
      () => stateValue.value,
      (val) => {
        hsl.value = { h: val.h, s: Math.round(val.s * 100), l: Math.round(val.v * 100) }
    ...
    const renderPopup = () => {
      return h(
        'div', { class: 'of-colorpicker-popup' },
        h(
        ...
        return [
          h(
            'div',
            {
              ...
              style: { backgroundColor: 'hsl(' + hsl.value.h + ', ' + hsl.value.s + '%, ' + hsl.value.l + '%)' },
              id: inputId.value,
              tabindex: 0,
            },
            value
